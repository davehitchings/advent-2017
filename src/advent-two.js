...
export function adventTwoPartOne(arr) {
  let total = 0;

  arr.forEach((subArr) => {
    total += Math.max(...subArr) - Math.min(...subArr);
  });

  return total;
}

export function adventTwoPartTwo(arr) {
  let total = 0;

  arr.forEach((subArr) => {
    subArr.some((numerator) => {
      const denominator =
        subArr.find(item => numerator !== item && (numerator / item) % 1 === 0);
...
console.log('Answer:', adventTwoPartOne(input));

const partTwoInputTest = [
  ['5', '9', '2', '8'],
  ['9', '4', '7', '3'],
  ['3', '8', '6', '5'],
];
...
console.log('Tests:');
test(adventTwoPartTwo(partTwoInputTest), 9, 'test');
console.log('Answer:', adventTwoPartTwo(input));
